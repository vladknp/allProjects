$(function(){
  ...
  //=====  Forms  =====//
  //
  var recipient = 'nyrievrd@yandex.ru';
  ...
  setInterval(function () {
    ...
    //Set an and date
      ...
      var offer_day = ("Акция действует до " + offer.getDate() + "." + (offer.getMonth()+1) + "." + (offer.getFullYear()-2000));
...
//
function clickMore() {
  var inner = document.getElementsByClassName("training-block-1");
  for (i=0; i<inner.length; i++) {
    if (inner[i].style.display == "inline-block"){
      inner[i].style.display = "none"
    }
    else
      inner[i].style.display = "inline-block"
  }

  var inner = document.getElementsByClassName("training-block-2");
  for (i=0; i<inner.length; i++) {
